...
SCENARIO_DIR="$ROOT_DIR/scenarios/01-oldschool-pipe"
LOG_DIR="$SCENARIO_DIR/logs"
COMPOSE_FILE="$ROOT_DIR/docker/docker-compose.yml"

TOPIC_NAME="${TOPIC_NAME:-RAW_PIPE}"
...
mvn -f "$ROOT_DIR/pom.xml" -pl shared-model,scenarios/01-oldschool-pipe -am clean package
...
producer_exit_code="$(wait_for_container_exit "kb-producer" "$deadline_epoch")"
consumer1_exit_code="$(wait_for_container_exit "kb-consumer-1" "$deadline_epoch")"
consumer2_exit_code="$(wait_for_container_exit "kb-consumer-2" "$deadline_epoch")"
...
{
  echo "ENV"
  echo "scenario=oldschool-pipe"
  echo "java_runtime=eclipse-temurin:17-jre"
  echo "topic_name=$TOPIC_NAME"
  echo "record_count=$RECORD_COUNT"
  echo "slot_count=250"
  echo "consumer_1_group=kb-oldschool-c1"
  echo "consumer_2_group=kb-oldschool-c2"
  echo
  echo "RUNNER_TIMING"
