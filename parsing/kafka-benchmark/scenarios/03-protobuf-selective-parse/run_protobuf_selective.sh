...
SCENARIO_DIR="$ROOT_DIR/scenarios/03-protobuf-selective-parse"
LOG_DIR="$SCENARIO_DIR/logs"
COMPOSE_FILE="$ROOT_DIR/docker/docker-compose.scenario3.yml"

TOPIC_NAME="${TOPIC_NAME:-PROTO_FULL}"
...
mvn -f "$ROOT_DIR/pom.xml" -pl shared-model,scenarios/03-protobuf-selective-parse -am clean package
...
producer_exit_code="$(wait_for_container_exit "kb3-producer" "$deadline_epoch")"
consumer1_exit_code="$(wait_for_container_exit "kb3-consumer-1" "$deadline_epoch")"
consumer2_exit_code="$(wait_for_container_exit "kb3-consumer-2" "$deadline_epoch")"
...
{
  echo "ENV"
  echo "scenario=protobuf-selective-parse"
  echo "java_runtime=eclipse-temurin:17-jre"
  echo "topic_name=$TOPIC_NAME"
  echo "record_count=$RECORD_COUNT"
  echo "slot_count=250"
  echo "consumer_1_group=kb-proto-c1"
  echo "consumer_2_group=kb-proto-c2"
  echo
  echo "RUNNER_TIMING"
